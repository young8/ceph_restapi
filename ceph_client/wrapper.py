...
import ceph_client.client as client
import ceph_client.exceptions as exceptions
...
class CephWrapper(client.CephClient):
    ...
    # osd GET calls
    ###
    def osd_find_metadata(self, id, **kwargs):
    ...
    def osd_pool_ls(self, epoch=None, **kwargs):
        if epoch is not None:
            return self.get('osd/pool/ls?detail'
                            .format(epoch), **kwargs)
        else:
    ...
    # pg PUT calls
    ###
    def set_full_ratio(self, ration, **kwargs):
        return self.put('pg/set_full_ratio?ratio={0}'
                        .format(ration), **kwargs)

    def set_nearfull_ratio(self, ration, **kwargs):
